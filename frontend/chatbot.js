(() => {
  ...
  const basketLink = el('a', '', 'Корзина →'); basketLink.href = './basket.html'; toolbar.append(reset, refresh, basketLink);
  ...
  dialog.append(header, toolbar, history, context, files, status, form, footnote);
  document.body.append(launcher, dialog);
  let sessionId, busy = false, selectedProduct = null, uploaded = [], pending = null, snapshot = null, opening = null, connectionEpoch = 0;
  const storageKey = () => `skyline-product-chat:${window.Shop.base}`;
  function restoreId() { try { sessionId = localStorage.getItem(storageKey()); } catch { sessionId = null; } }
  function remember() { try { if (sessionId) localStorage.setItem(storageKey(), sessionId); } catch { /* Storage is optional. */ } }
  restoreId();
  function setStatus(text = '', error = false) { status.textContent = text; status.classList.toggle('error', error); }
  function setBusy(value) { busy = value; send.disabled = reset.disabled = refresh.disabled = attach.disabled = value; input.disabled = value;
  ...
  async function ensureSession() {
    ...
    if (!opening) opening = api('/api/product-chat/sessions', { method: 'POST' }).then(data => { sessionId = data.id; remember(); return sessionId; }).finally(() => { opening = null; });
    return opening;
  }
  async function reload() {
    if (!sessionId) { welcome(); return; }
    try { render(await api(`/api/product-chat/sessions/${sessionId}`)); }
    catch (error) { if (error.status === 404) { sessionId = null; snapshot = null; welcome(); } else throw error; }
  }
  async function open() {
    if (!dialog.open) dialog.showModal();
    launcher.setAttribute('aria-expanded', 'true');
    if (!busy) { setStatus(''); try { await reload(); } catch (error) { setStatus(error.message, true); } }
    input.focus();
  }
  ...
  refresh.addEventListener('click', async () => { if (busy) return; setBusy(true); setStatus(''); try { await reload(); } catch (error) { setStatus(error.message, true); } finally { setBusy(false); } });
  reset.addEventListener('click', () => { if (busy) return; sessionId = null; snapshot = null; pending = null; uploaded = []; selectedProduct = null; context.hidden = true; input.value = '';
    try { localStorage.removeItem(storageKey()); } catch { /* Storage is optional. */ } renderFiles(); welcome(); setStatus(''); input.focus(); });
  window.addEventListener('ask-product', async event => { selectedProduct = event.detail.id; context.textContent = `Товар: ${event.detail.name}`; context.hidden = false; await open(); input.value = `Расскажите о товаре «${event.detail.name}»`; input.focus(); });
  window.addEventListener('shopping-api-changed', () => { connectionEpoch++; sessionId = null; snapshot = null; pending = null; uploaded = []; selectedProduct = null; context.hidden = true; restoreId(); renderFiles(); welcome(); if (dialog.open && !busy) open(); });
  welcome();
})();
